...
query = (
    spark.readStream.schema("id INT, timestamp TIMESTAMP, count INT, hash STRING")
    .json(FILES_PATH)
    .groupBy("id")
    .agg(F.avg("count").alias("avg_count"), F.min("timestamp").alias("min_timestamp"))
...
df = spark.read.format("statestore").load(CHECKPOINT_PATH).display()
